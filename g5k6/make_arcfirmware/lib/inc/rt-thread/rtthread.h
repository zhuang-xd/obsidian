...
#ifdef FH_USING_RUNLOG
struct trace_node;
extern struct trace_node *g_trace_list;
extern void runlog_init(void);
extern void runlog_add_node(unsigned int para0, unsigned int para1, unsigned int para2, char* name);
#define RUNLOG_INIT() runlog_init()
#define RUNLOG_ADD(para0,para1,para2,name) do{if(g_trace_list) runlog_add_node((unsigned int)(para0),(unsigned int)(para1),(unsigned int)(para2),(char*)(name));}while(0)
#else
#define RUNLOG_INIT() do{}while(0)
#define RUNLOG_ADD(para0,para1,para2,name) do{}while(0)
#endif
