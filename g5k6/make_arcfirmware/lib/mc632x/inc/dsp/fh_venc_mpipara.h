...
#define VENC_MAX_CHN_NUM	(32) // MAX channel num
...
typedef enum {
	FH_HARD_MODE   = 0,// 硬件直连模式 | [ ]
	FH_NORMAL_MODE   = 1,// 正常工作模式 | [ ]
	FH_HARD_ALL_MODE   = 2,// reserved
	FH_NORMAL_ALL_MODE   = 3,// reserved
} FH_ENC_WORK_MODE;
...
typedef struct
{
	...
	FH_ENC_WORK_MODE work_mode;        // 工作模式
}FH_VENC_CHN_CAP;
...
typedef struct
{
	FH_UINT32 info0;   //保留属性
	FH_UINT32 info1;   //保留属性
	FH_UINT32 info2;   //保留属性
	FH_UINT32 info3;   //保留属性
	FH_UINT32 reserved[2];   //保留属性
}FH_H26x_AOV_STREAM_INFO;
...
typedef struct
{
	...
	union
	{
		...
		FH_H26x_AOV_STREAM_INFO aov_info;          // AOV码流统计信息 | [ ]
	};
}FH_VENC_STREAM;
...
//add from ourVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV
typedef struct
{
	FH_UINT32 luma_addr;
...
{
	FH_UINT32 chan;
	FH_VIDEO_FRAME stVencSendframe;
	FH_UINT32 timeout_ms;
}FH_VENC_SEND_STREAM_EX;
...
{
	FH_UINT32 chan;
	FH_VENC_CHN_PARAM stVencChnParam;
}FH_VENC_SET_CHN_PARAM;
...
typedef struct
{
	...
	FH_UINT32 strJpegPhyAddr;
	FH_ADDR strJpegVirAddr;
	FH_UINT32 strJpegSize;
}FH_VENC_STREAM_ADDR;
...
{
	FH_UINT32 chan;
	FH_H264_VUI stVencH264Vui;
}FH_VENC_H264VUI_ATTR;

typedef struct
{
	FH_UINT32 chan;
	FH_H265_VUI stVencH265Vui;
...
typedef struct
{
	FH_UINT32 chan;
	FH_UINT32 cmprate;
}FH_VENC_CMPRATE_ATTR;

typedef struct
{
	FH_UINT32 chan;
	FH_UINT32 graymode;
}FH_VENC_GRAYMODE_ATTR;

typedef struct
{
	FH_UINT32 chan;
	FH_UINT32 osdmode;
}FH_VENC_OSDMODE_ATTR;


//add from our AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
{
	FH_UINT32 chan;
	FH_VENC_CHN_CAP stVencChnCap;
}FH_VENC_CHN_CAP_ATTR;

typedef struct
{
	FH_UINT32 chan;
	FH_UINT64 timestamp;
}FH_VENC_CHN_CURTIME_ATTR;
