...
#define FHT_OSD_GRAPH_CTRL_CUSTOM_BEFORE_VP (1<<4)
...
#define FHT_OSD_GRAPH_CTRL_CUSTOM_AFTER_VP (1<<20)

#define FHT_OSD_DEBUG_LEVEL_OFF   (0) /*关闭debug功能*/
#define FHT_OSD_DEBUG_LEVEL_ERROR (1 << 0) /*打印错误信息*/
#define FHT_OSD_DEBUG_LEVEL_WARN  (1 << 1) /* 打印警告日志 */
#define FHT_OSD_DEBUG_LEVEL_INFO  (1 << 2) /* 打印信息日志 */
#define FHT_OSD_DEBUG_LEVEL_DBG   (1 << 3) /* 打印调试日志 */
#define FHT_OSD_DEBUG_LEVEL_ALL   (0xf) /*打印所有相关信息*/
...
/*********
 * 函数名：  FHAdv_Osd_VctFont_Init
 * 函数功能: 矢量字库初始化，并加载矢量字库
 * 输入参数: szFile: 矢量字库文件名，如“simsun.ttc”
 * 输出参数: 无
 * return:  见错误码ADV_ERR_XXX
 * 注意： 这个接口只能在linux上使用，RTos系统由于缺少开源库freetype，暂未支持。
 *
 *********/
FH_SINT32 FHAdv_Osd_VctFont_Init(const char *szFile);
 ...
 *功能: 设置OSD时间偏移.
 *参数: vpuId: vpu group id, 范围： 0 - 256
 *      timeoffset, osd时间显示偏移，基于gettimeofday()的时间偏移量，单位秒.
 *返回值: 见错误码ADV_ERR_XXX
 */
FH_SINT32 FHAdv_Osd_SetTimeOffset(FH_SINT32 vpuId, FH_SINT32 timeoffset);
...
FH_VOID FHAdv_Osd_Print_Level_Config(FH_UINT32 debug_level);
...
/*
 * 功能: 用户提供自定义的内存用于osd显示
 * 输入参数: vpuId: vpu group id, 范围： 0 - 256
 *      chn,通道号,如果初始化时custom图层被安排在VPU分通道前,那么chn填0即可,如果被安排在VPU分通道后,那么
 *          对于主码流,其chn等于0; 对于子码流,其chn等于1
 *       graphinfo, 和FH_VPSS_SetGlbGraphV2参数相同，用于配置graph信息, logo_idx配置是无效的。
 *返回值: 0 成功  -1 通道号不对或者配置参数指针为空
 * 说明: 这个接口和FH_VPSS_SetGlbGraphV2作用是一样的，区别在于使用FH_VPSS_SetGlbGraphV2时不能同时使用advapi的osd接口，
 * 而这个接口可以做到与FH_VPSS_SetGlbGraphV2效果一样的同时兼容advosd接口。
 */
FH_UINT32 FHAdv_Osd_User_Custom(FH_UINT32 vpuId, FH_UINT32 chan, FH_VPU_LOGOV2 *graphinfo);
