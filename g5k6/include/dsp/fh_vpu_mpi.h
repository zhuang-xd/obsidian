...
*          非0(失败，详见错误码)
*   Note:
*          需要在FH_VPSS_Suspend后调用,且用户需保证FH_VPSS_Suspend和FH_VPSS_Resume期间vpu不能工作,不然硬件现场会改变
*/
FH_SINT32 FH_VPSS_Resume(FH_UINT32 grpid);
...
*          非0(失败，详见错误码)
*   Note:
*          专为AOV下提供,不带任何阻塞的FH_VPSS_Disable,以及保存现场
*/
FH_SINT32 FH_VPSS_Suspend(FH_UINT32 grpid);
...
*          非0(失败，详见错误码)
*   Note:
*          仅支持VPU_CROP_SCALER,且crop幅面大于等于当前通道幅面的情况，否则会返回参数错误。
*          如果切了通道幅面导致crop幅面小于通道幅面，会取消之前设置的crop，需要重新设置。
*          各平台支持功能有所差异,详见《视频函数开发参考手册》。
*          需要在FH_VPSS_SetChnAttr后调用。
*/
...
*          非0(失败，详见错误码)
*   Note:
*          MC632x仅通道2支持,sel: 0->fix2ds之后 1->fix2ds之前 2 gosd之后
*/
...
*          此接口适用于FH8852V201系列＆FH8626V200&MC632x产品,仅通道2支持。
*          需要在FH_VPSS_CreateGrp后调用。
*/
