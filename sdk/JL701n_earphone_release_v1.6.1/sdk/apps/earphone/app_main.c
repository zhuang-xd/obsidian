...
/**@brief   开关检测函数
   ...
   @note    void sw_init_api()  sw_check()
*/
/*----------------------------------------------------------------------------*/
void sw_init_api(void)
{
    gpio_set_direction(SW_CHECK_IO, 1);
    //gpio_set_hd(SW_CHECK_IO, 0);
    //gpio_set_hd0(SW_CHECK_IO, 0);
    gpio_set_pull_up(SW_CHECK_IO, 0);
    gpio_set_pull_down(SW_CHECK_IO, 1);
    gpio_set_die(SW_CHECK_IO, 1);
    //printf("sw_init_api OK\n\n");
}

void sw_check(void *param)
{
    static u8  sw_in_cnt;
    static u8  sw_out_cnt;
    sw_init_api();
    if(gpio_read(SW_CHECK_IO))
    {
        sw_out_cnt = 0;
        if(sw_in_check_flag==0)
        {
            sw_in_cnt++;
        }
        if (sw_in_cnt > 5)
...
#ifdef SW_CHECK_EN
    sys_s_hi_timer_add(NULL, sw_check, 10);
#endif
